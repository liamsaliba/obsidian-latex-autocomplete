...
class LatexSuggest extends EditorSuggest<Definition> {
	plugin: LatexSuggestPlugin;
	pattern: RegExp;
	lastEditorSuggestTriggerInfo: EditorSuggestTriggerInfo;

	constructor(plugin: LatexSuggestPlugin) {
		super(plugin.app);
		this.plugin = plugin;
		this.pattern = /.*\\(\S+)$/;
	}
	...
	getSuggestions(context: EditorSuggestContext): Definition[] {
		return Object.keys(dict)
			.filter((v) => v.includes(context.query))
			.map((v) => ({
				Shortcut: v,
				Symbol: dict[v],
			}))
...
export default class LatexSuggestPlugin extends Plugin {
	async onload() {
		this.registerEditorSuggest(new LatexSuggest(this));
	}
}
